...
func TestAddTodoSuccess(t *testing.T) {
	...
	if err := json.Unmarshal(body, &rspMsg); err != nil {
		t.Fatal("Response type is not correct")
...
func TestDeleteListSuccess(t *testing.T) {
	...
	if err := json.Unmarshal(body, &rspMsg); err != nil {
		t.Fatal("Response type is not correct")
	} else if rspMsg.Status != "success" || rspMsg.Message != "deleted" {
		t.Fatal("Response type or values is not correct")
	}
}

// For TestDeleteTodoSuccess() & TestDeleteListSuccess()
// Need two todos at least
func TestAddTodoSuccess2(t *testing.T) {
	api := url + "/api/v1/event"
